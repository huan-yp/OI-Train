...
bool cmp(int x,int y) {
    return siz[y] * lef[x] > siz[x] * lef[y] ; 
}
int f[N] ; 
void dfs(int x) {
    siz[x] = 1 ;
    if(son[x].size( ) == 0) lef[x] = 1;
    for(int p:son[x]) dfs(p) , siz[x] += siz[p] , lef[x] += lef[p] , f[x] += f[p] ;
    sort(All(son[x]),cmp); 
    ll tot = 0 ; 
    for(int p:son[x]) {
        f[x] += (tot + 1)* lef[p] ;
...
signed main()
{
    FileIO(street);
    cin >> n ; 
    rep(i,2,n) cin >> fat >> c[i] , son[fat].emplace_back(i);
    dfs(1) ; 
    D res = f[1] / (1.00 * lef[1]); 
    printf("%.8lf" , res) ;
    return 0;
}
