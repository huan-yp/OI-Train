...
void Divm() {
    for(int i = 2 ; i * i <= m ; ++ i) {
        if(m % i == 0) fct.emplace_back(i);
        while(m % i == 0) m /= i , spc[(int) (fct.size()) - 1] ++;
    }
    if(m != 1) fct.emplace_back(m) , spc[(int) (fct.size()) - 1] ++; 
}
void clc() {
    for(int i = 1 ; i <= n ; ++ i) {
        int ff = i ;
        for(int p = 0 ; p < fct.size( ) ; ++ p) {
            f[i][p] = f[i-1][p] ;
            while(ff % fct[p] == 0) ff /= fct[p] , f[i][p] ++ ;
        }
    }
}
bool solve(int id) {
    rep(ff,0,cnt) rl[ff] = f[n][ff] - f[id][ff] - f[n-id][ff] ;  
    bool flg = 1 ; 
    rep(ff,0,cnt) if(rl[ff] < spc[ff]) flg = 0 ;
    return flg ; 
}
signed main()
{
    ...
    clc( );
    for(int i = 0 ; i <= n ; ++ i) {
        if(solve(i)) ret.emplace_back(i + 1) ;
    }
    wrt(ret.size() , '\n') ;
    for(int p:ret) wrt(p,' ') ;
    return 0;
